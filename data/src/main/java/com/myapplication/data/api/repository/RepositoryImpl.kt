package com.myapplication.data.api.repository
...
class RepositoryImpl @Inject constructor(private val userStorage: AutoLoginStorage,private val remoteDataSource: RemoteDataSource):BaseApiResponse(){
    ...
    fun saveStorageToken(token:AutoLoginTokenReceive):Boolean{
        return userStorage.saveToken(token)
    }
    fun getStorageToken():AutoLoginTokenReceive{
        return userStorage.getToken()
    }
    fun saveLoginAndInfo(tokenResponse: AutoLoginTokenResponse):Boolean{
        return userStorage.saveLoginAndInfo(tokenResponse)
    }
    fun getLoginAndIngo():AutoLoginTokenResponse{
        return userStorage.getLoginAndInfo()
    }
