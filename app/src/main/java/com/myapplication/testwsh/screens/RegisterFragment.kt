...
class RegisterFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        System.out.println("ssssss")
        System.out.println(vmreg.mylogin.value.toString())
        System.out.println(vmreg.mylogin.value?.length)
        System.out.println("ssssss")
        vmreg.autologin()
        vmreg.responseAutoLog.observe(requireActivity() as MainActivity, Observer {
            if (it!=null){
                initstateAfterAutoLogIn()
            }
        })
    ...
    fun saveNamesAndLogin(){
        ...
        activity?.findNavController(R.id.nav_host)?.navigate(R.id.registerCreatePasswordFragment)
    }
    fun funClickBtnSign(){
